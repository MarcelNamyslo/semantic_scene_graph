"""This is a module that acts as a Command Line Interface (CLI) for the semantic-scene-graph project"""
...
def call_controller(
    csv_path: str, osm_path: str, x_origin, y_origin, timestamps, clean_load, verbose=False
):
    """Calls the init function of controller.Controller and starts the program by invoking its main function passing the provided parameters.
    Times the whole process and prints the time.
    ...
    Args:
        ...
        timestamps (list(int)): The timestamps to be computed. If an empty list is provided, all timestamps of the dataset are computed.
    ...
    controller.main(csv_path, osm_path, origin, timestamps, clean_load, verbose)
...
class ListAliasesAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string, **kwargs):
        """Runs the list_aliases action. Reads all saved aliases from the csv file at ALIAS_FILE_PATH and prints them and their parameters.
...
class UseAliasAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string, **kwargs):
        """Runs the alias action. Loads a saved aliases from the csv file at ALIAS_FILE_PATH and runs call_controller() with its parameters.
        Exits the parser in the end. Ignores every argument except --alias (-a) and the timestamps after it.

        Args:
            parser (argparse.ArgumentParser): The used parser.
            namespace (argparse.Namespace): The namespace containing all used arguments.
            values (list): The list of values. Here the alias string followed by the wanted timestamps.
            option_string (str): The string of the option. Should be "--list_aliases" always
        """
        alias = values.pop(0)
...
class Formatter(argparse.HelpFormatter):
    ...
    def _format_args(self, action: argparse.Action, default_metavar: str) -> str:
        ...
        Args:
            action (argparse.Action): The action this formatter is used on. Only changes the return if the alias action is provided.
            default_metavar (str): The default metavar. See super()._format_args.

        Returns:
            str: calls the super function if the action is not alias. If it is it returns the formatted help string.
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "csv_path",
        help="the csv file location. The csv file can either be formatted in the TAF format or in the inD format.",
    )
    parser.add_argument(
        "osm_path",
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=int,
        help="all timestamps that are supposed to be calculated. Default if left empty is all timestamps in the csv file.",
        default=[],
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save_alias",
        type=str,
        help="save the given csv, osm and origin information so it can be used with the alias later",
        metavar="ALIAS",
    )
    parser.add_argument(
        "-c",
    ...
    parser.add_argument(
        ...
        metavar="ALIAS",
        type=str,
        help="loads the dataset of the given alias and creates its Semantic Scene Graphs for either the provided timestamps or the whole scenario. This option has to be the last option if it is used.",
        action=UseAliasAction,
    )
    # Parse the provided arguments
    args = parser.parse_args()

    # Check if the save_alias action is called and writes the alias and the parameters to the csv file at ALIAS_FILE_PATH
    if args.save_alias is not None:
        ...
        if args.save_alias not in alias_column.values:
            ...
            print(
                "The provided alias has already been assigned. Please use a new alias and try again."
            )
            parser.exit()
