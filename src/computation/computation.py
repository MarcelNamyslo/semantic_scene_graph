...
class ProbabilisticMatchingDict(dict):
    ...
    def __init__(
        self,
        scene: loader_module.Scene,
        lanelet_map: lanelet2.core.LaneletMap,
        von_mises_kappa=2.0,
        max_dist=1.0,
    ):
        """Creates a probabilistic matching dictionary by matching Entities to lanelets of the traffic scene.
        Each entity is matched to a lanelet based on its current position and yaw on a lanelet, the covariance of its position and the kappa of the
        von Mises distribution.
        The matches have to be in range of a maximum euler distance in order to be considered as a match.
        The probabilistic matches are later used to create projection identities, which help to determine relations between the distinct entities.

        Args:
            scenario(loader_module.Scenario): Scene object depicting every Entity for one timestamp (every EntityState matching one timestamp).
            timestamp (int): time at which the traffic participants are to be matched to the lanelets
            von_mises_kappa (int, optional): Kappa of the von Mises distribution. Defaults to 2.
            max_dist (int, optional): Euclidian distance threshold for matching. Defaults to 1.
        """
        ...
        for entity_state in entity_states:
            ...
            current_obj_with_cov = lanelet2.matching.ObjectWithCovariance2d(
                ...
                lanelet2.matching.Pose2d(entity_state.x, entity_state.y, entity_state.yaw),
                [],
...
class ProjectionIdentityDict(dict):
    ...
    The outermost dictionary uses entity_id as keys with the corresponding value being a dictionary with lanelet ids as keys and the probability of the match as value.
    This looks like this:
    >>> {entity_id: {lanelet_id: matching_probability}}
    Example:
    >>> {some_entity_id: {some_lanelet_id: matching_probability0, some_other_lanelet_id: matching_probability1}}
    """
    ...
    def __init__(
        ...
        This is achieved by dividing the given mahalanobis distance of each match through the sum of all mahalanobis distances of the matches corresponding to the entity.

        Args:
            matching_dict (dict[int, list[lanelet2.matching.ConstLaneletMatchProbabilistic]]): Should contain entity_id as keys and a list of all possible matches as their value.
            verbose (bool, optional): Set to True for additional output. Defaults to False.
        """
        # took the absolute value of the mahalanobisDistSq, so the probability should make more sense now. Before values above 1 were possible.
        self.__keys_swapped = {}
        for entity_id in matching_dict:
            ...
            for match in matching_dict[entity_id]:
                ...
                self[entity_id][lanelet.id] = probability
    ...
    def swap_keys(self) -> dict[int, dict[int, float]]:
        """Same dict-of-a-dict structure as ProjectionIdentityDict but with entity id and lanelet id swapped.

        Returns:
            dict[int, dict[int, float]]: First key is entity id, second key is lanelet id and the contained value is the probability.
        """
...
class Roadgraph:
    ...
    def __init__(
        ...
        """Create Roadgraph by first importing a premade set of traffic_rules amd then creating the RoutingGraph using the lanelet_map and the
        traffic_rules.

        The graph is converted to the igraph format by using a workaround. The routing_graph is first exported in GraphML format and then
        imported in igraph format. This is done because igraph is currently not supporting reading the .dot format.

        Lastly all shortest paths between lanelets containing a match are calculated.
        A path is defined by a list of continuous lanelets in the order in which they are traversed to reach the destination.

        Args:
            lanelet_map (lanelet2.core.LaneletMap): map generated by lanelet2 from an osm file and the coordinates of the road segment represented
                in the map
            matching_dict (ProbabilisticMatchingDict): dict containing entity_ids and their list of matches
        """
        self._graph = graph

        self._lanelet_dict = {lanelet.id: lanelet for lanelet in lanelet_map.laneletLayer}

        self._matching_dict = matching_dict
        self._compute_shortest_relation_paths(lanelet_map, matching_dict, verbose)
    ...
    def _compute_shortest_relation_paths(self, lanelet_map, matching_dict, verbose):
        ...
        # The matched Lanelets + the Lanelets that are part of an conflicting / overlapping relation
        targeted_igraph_ids = list({*matched_lanelet_igraph_ids, *overlapping_igraph_ids})
        ...
        We are searching in a roadgraph without conflicts because we want to avoid conflicting relations in the middle of paths
        as they can result in non-valid paths. See dummy Lanelets and __merge_paths function for more information.
        ...
        So if there is a path 1->2->3->4 and all relations were successors, the dict entry would be:
        ...
        self.__shortest_relation_path_dict = {
            lanelet_id: {
                self._lanelet_id(targeted_igraph_ids[index]): [
                    (
                        self._lanelet_id(graph_without_conflicts.es[edge_id].tuple[0]),
                        graph_without_conflicts.es[edge_id]["relation"],
                        graph_without_conflicts.es[edge_id]["routingCost"],
                        self._lanelet_id(graph_without_conflicts.es[edge_id].tuple[1]),
                    )
        ...
        # List of tuples of lanelet ids (int, int) that describe all conflicting relations. These have been deleted from the graph_without conflicts
        conflicting_relations = [
            (self._lanelet_id(edge.tuple[0]), self._lanelet_id(edge.tuple[1]))
            for edge in conflicting_edges
        ]

        # Re-append conflicting relations to the end of the paths in shortest_relation_path_dict if the last lanelet
        # of the path is the same as the first lanelet of the conflicting relation.
        # This has to be done because we removed all conflicting relations before searching for shortest paths.
        # TODO: optimize runtime-wise. Paths only consisting of a conflicting relation are currently not respected.
            ...
            for (
                ...
                for path in to_lanelet_and_path_list.values():
                    ...
                    if path and path[-1][-1] == edge_tuple[0] and path[-1][1] != "Conflicting":
                        ...
                        new_path = [(edge_tuple[0], "Conflicting", 1.0, edge_tuple[1])]
                        path_found = True
        ...
        merged_paths = []
        # All paths to one specific igraph id (the equivalent lanelet id) are selected and merged by self.__merge_paths()
        ...
        # The merged paths are added to the shortest_relation_path_dict if they are shorter than the existing paths.
        # This way we also choose the shortest path between two lanelets from all the merged paths.
        # It has to be done outside of __merge_paths so that the already merged paths are not selected as a path to merge.
        # Doing this inside of __merge_paths would also result in missing out on some shortest paths.
            ...
            for overlapping_igraph_id in overlapping_igraph_ids:
                ...
                if self._lanelet_id(overlapping_igraph_id) not in matched_lanelet_ids:
                    ...
                    if dummy_lanelet in shortest_path_dict.keys():
                        ...
                        if dummy_lanelet in matched_lanelet_ids:
                            print("alarm!")
                        elif shortest_path_dict[dummy_lanelet] and verbose:
                            print("deletes path")
    ...
    def __merge_paths(self, path_list) -> list[list[tuple[int, str, float, int]]]:
        """Merges the provided paths and adds them to the shortest_relation_path_dict if they are shorter than the previously calculated ones.
        All provided paths have to lead to the same Lanelet. These paths are then split up into paths that end with a "Conflicting" relation and those that don't.
        In the next step they are merged and inserted into the shortest_relation_path_dict after assuring that they are actually shorter.
        Finally all provided unmerged paths are deleted from shortest_relation_path_dict if the Lanelet they lead to is a dummy Lanelet.
        TODO: explain what a dummy lanelet is.
        A dummy lanelet is a lanelet which has an intersecting relation with another lanelet and is used to create a path between two lanelets A & B.
        This path is created by merging the path from lanelet A to the dummy lanelet and the path from lanelet B to the dummy lanelet.



        Args:
            path_list (list[int, list[tuple(int, str, float, int)]]): The List of paths to be merged, all leading to the same Lanelet.

        Returns:
            calculated_paths (list[list[tuple(int, str, float, int)]]): The List of the merged paths. These still have to be added to the shortest_relation_path_dict
        """
        ...
        for overlapping_path in overlapping_paths:
            ...
            for non_overlapping_path in non_overlapping_paths:
                # the path from the target lanelet to the dummy lanelet is reverted to get a directed path from
                # the origin lanelet to the dummylanelet to the target lanelet
                # (from_lanelet, relation, routing_cost, to_lanelet) becomes (to_lanelet, relation, routing_cost, from_lanelet)
                temp_non_overlapping_path = [
                    (sub[3], sub[1], sub[2], sub[0]) for sub in non_overlapping_path[1]
                ]
                temp_non_overlapping_path.reverse()
                intersecting_path = overlapping_path[1] + temp_non_overlapping_path
    ...
    def __first_path_longer(self, first_path, second_path) -> bool:
        ...
        Args:
            first_path (list[int, list[tuple(int, str, float, int)]]): the first path to be measured
            second_path (list[int, list[tuple(int, str, float, int)]]): the second path to be measured
    ...
    def __calculate_path_length(self, path) -> float:
        """Calculates the length of a path based on the edge weight (routingCost provided by Lanelet2)
    ...
    def get_shortest_relation_path(
        ...
        """Shortest relation path from a lanelet of the roadgraph to another lanelet in the roadgraph.
    ...
    def get_all_shortest_relation_paths(
        ...
        """Shortest relation paths from a lanelet of the roadgraph to all possible lanelets in the roadgraph.

        Args:
             from_lanelet (lanelet2.core.ConstLanelet): Origin lanelet.

         Returns:
             dict[int, list[int]]: Dictionary of shortest paths represented by a list roadgraph edge relations.
        """
...
class EdgeMode(Enum):
    """An Enum for the possible modes for which edges in the SemanticSceneGraph are kept if there are double edges due to multiple ProjectionIdentities per Entity"""
...
class SemanticSceneGraph:
    ...
    the semantic scene graph model is utilized to describe traffic scenes in relation to road topology and to compare different scenes with each other,
    independent of the location. The relations between traffic participants are described by semantically classified edges.
    ...
    def __init__(
        ...
        for attribute_name in scene.entity_states[0].attribute_dict.keys():
            self.__graph.vs[attribute_name] = [
                scene.get_entity_state(entity_id).attribute_dict[attribute_name]
                for entity_id in matching_dict.keys()
            ]
        # Add projection identity to the nodes as string
        self.__graph.vs["projection_identity"] = [
            str(projection_identity_dict[entity_id]).replace("{", "").replace("}", "")
            for entity_id in matching_dict.keys()
        ]
        # Add edges. Three types of directed edges have to be added. All three types of edges start at the
        # vertex node of the relation path and end at the last vertex of the relation path.
        # Let x ∈ ℕ0
        # A longitudinal type edge is added if the roadgraph relation path contains x consecutive relations.
        # A lateral type edge is added if the roadgraph relation path contains x consecutive relations and exactly
        # one adjacent relation.
        # An intersecting type edge is added if the roadgraph relation path contins x consecutive or adjacent relations
        # and one overlapping relation. Note that once an edge with the attribute overlapping is in path from a to b,
        # all subsequent edges must be reversed.
                    ...
                    for target_entity_id in projection_identity_dict.swap_keys[
                        ...
                        if origin_entity_id != target_entity_id:
                            ...
                            if relation_path_relations.count(overlapping) == 1:
                                ...
                                if path_distance[1]:
                                    self.__graph.add_edges(
                                        [
                                            (
                                                self.__graph.vs.find(id=origin_entity_id),
                                                self.__graph.vs.find(id=target_entity_id),
                                            )
                                        ],
                                        attributes={
                                            "label": "int",
                                            ...
                                            "centerline_distance_origin": self.distance_from_centerline(
                                                origin_entity_id, origin_lanelet_id
                                            ),
                                            "centerline_distance_target": self.distance_from_centerline(
                            ...
                            # lateral
                                ...
                                relation_path_relations.count(adjacent_right) == 1
                            ) and overlapping not in relation_path_relations:  # TODO: remove this line if tests work without it
                                path_distance = self.calculate_path_distance(
                                    origin_lanelet_id,
                                    target_lanelet_id,
                                    origin_entity_id,
                            ...
                            # longitudinal or Entities are on the same lanelet. This is also a longitudinal relation from the further back entity to the one in front.
                            elif (
                                ...
                                and overlapping
                                not in relation_path_relations  # TODO: remove this line if tests work without it
                            ...
                            # This shouldn't really happen. But if it does, print the path as an error.
        ...
        if verbose:
            print(
                f"There are now {len(self.__graph.es)} edges left in the graph. {edge_count-len(self.__graph.es)} double edges have been removed because of the edge filter mode {edge_mode}."
            )
    ...
    def remove_double_edges(self, graph: igraph.Graph, edge_mode: EdgeMode):
        ...
        """
        if edge_mode != EdgeMode.KEEP_ALL and len(graph.es) > 0:
            edge_dict = {}
            for edge in graph.es:
                origin_entity_id = edge.tuple[0]
                target_entity_id = edge.tuple[1]
                if (origin_entity_id, target_entity_id) in edge_dict:
                    other_edge = edge_dict[(origin_entity_id, target_entity_id)]
                    ...
                    edge_dict[(origin_entity_id, target_entity_id)] = edge
            # We have to remove edges like this here because removing edges while storing other edge objects outside of the graph messes with the edges
    ...
    def calculate_path_distance(
        ...
        If the relation is "intersecting", the distance is only measured between the origin entity and the intersection point.
        ...
        if not relation_path:
            ...
            assert origin_lanelet_id == target_lanelet_id
            ...
            # negative if the target is behind the origin. This means there is no relation origin --Successor--> target
            distance = target_entity_location_arc.length - current_measurement_location_arc.length
            if distance < 0:
                path_is_valid = False
        ...
        for step in relation_path:
            ...
            if step[1] == "Conflicting":
                ...
                if relation != relation.INTERSECTING and self.__verbose:
                    # TODO: remove this and refactor everything in the for loop of the constructor after testing. Also remove the relation enum.
                    print("Intersecting relation not classified as one but as " + str(relation))
                intersection_points = lanelet2.geometry.intersectCenterlines2d(
                    lanelet, next_lanelet
                )
                if not intersection_points:
                    # No intersection of centerlines, calculate distance to the point where the centerlines are closest to each other instead
                ...
                # Check if the current measurement point has already passed the first intersecting location
                if (
                    ...
                    if (
                        ...
                        if self.__verbose:
                            print(
                                "Origin Entity has already passed the intersecting location. The current relation is not a valid relation and has to be removed."
                                + str(step)
                ...
                # TODO: check if target entity that is at least one lane change away from conflicting relation has passed the intersecting point
                if (
                    ...
                    # Entity has passed the first intersecting location. Set intersecting location to last intersecting location
                    intersection_location_arc = lanelet2.geometry.toArcCoordinates(
                        lanelet2.geometry.to2D(lanelet.centerline),
                        lanelet2.geometry.to2D(intersection_points[-1]),
                    )
                    ...
                    if (
                        ...
                        if self.__verbose:
                            print(
                                "Target Entity has already passed the intersecting location. The current relation is not a valid relation and has to be removed."
                                + str(step)
                                + str(origin_entity_id)
                                + ", "
                                + str(target_entity_id)
                ...
                # Do the lane change asap and not at the closest point because it can cause problems with multiple lane changes in a row
                current_measurement_point = lanelet2.geometry.project(
                    next_lanelet.centerline, current_measurement_point
                )
                # the arc coordinates of the point that has been projected on the next lanelet but in the context of the current lanelet
                ...
                # Calculate length of current lanelet - the part of the lanelet that is already behind the current measurement location
                distance = distance + (
                    lanelet2.geometry.length2d(lanelet) - current_measurement_location_arc.length
                )
        ...
        if (
            ...
            # the origin entity is actually in front of the target entity --> relation might not be valid.
            print(
    ...
    def distance_from_centerline(self, entity_id, entity_lanelet) -> float:
        ...
        return lanelet2.geometry.toArcCoordinates(
            lanelet2.geometry.to2D(self.__lanelet_dict[entity_lanelet].centerline),
    ...
    def get_probability(
        ...
        """Calculates the probability of the given edge based on the probabilities of the matching identities of the two matches the edge connects.
...
def read_to_lanelet_map(
    ...
    Args:
        ...
        origin (tuple[float, float]): coordinates to the road segment (must have a high level of accuracy)
        verbose (bool, optional): Set to True for additional output. Defaults to False.
    """
    ...
    if verbose:
        print(f"{len(error_list)} errors, {len([lane for lane in lanelet_map.laneletLayer])} lanes")
    return lanelet_map
